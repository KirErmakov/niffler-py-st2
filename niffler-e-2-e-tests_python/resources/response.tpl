...
<body>
<div>
    <pre><code>Status code: {% if data.responseCode is defined %}{{ data.responseCode }}{% else %}Unknown{% endif %}</code></pre>
</div>

{% if data.url is defined %}
<div>
    <pre><code>URL: {{ data.url }}</code></pre>
</div>
{% endif %}
...
{% if data.headers %}
    ...
    {% for name, value in data.headers.items() %}
        <div>
            <pre><code><b>{{ name }}</b>: {{ value|default("null") }}</code></pre>
        </div>
    {% endfor %}
    </div>
{% endif %}
